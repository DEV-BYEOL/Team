				...
				<div class="panel-body">
					<form role="form" action="/question/modify" method="post">
					 ...
					 <!--  p.346 추가 -->
						...
						<div class="form-group">
						<label>quesCateCode</label> 
                             						
							 <select class="form-control" name='quesCateCode' />
    <option value='delivery' <c:if test="${board.quesCateCode == 'delivery'}">selected</c:if>>배송</option>
    <option value='order' <c:if test="${board.quesCateCode == 'order'}">selected</c:if>>주문</option>
    <option value='else' <c:if test="${board.quesCateCode == 'else'}">selected</c:if>>기타</option>
</select>
				              </div>
